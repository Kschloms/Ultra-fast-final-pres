...
class M_torch:
    def __init__(self, Nc, eps, phi, wl, I, E0, res, device='cpu'):
        ...
        # Initialize the vector potential As
        vector = torch.stack([
            torch.zeros_like(self.ts, dtype=torch.float64),
            self.eps * torch.sin(self.wl * self.ts + self.phi),
            torch.cos(self.wl * self.ts + self.phi)
        ])
        self.As = (torch.sin(torch.pi * self.ts / self.T) ** 2) * self.A0 / torch.sqrt(torch.tensor(1 + self.eps ** 2, device=device, dtype=torch.float64)) * vector

    def f_phi_0(self, k):
        # k: (N, 3)
        k = k.to(dtype=torch.float64)
        k_mag = torch.norm(k, dim=-1)
        return 2 * torch.sqrt(torch.tensor(2.0, device=self.device, dtype=torch.float64)) / torch.pi * (1 / (k_mag ** 2 + 1) ** 2)
    ...
    def exp_integrand(self, k):
        ...
        N = k.shape[0]
        T = self.ts.shape[0]
        k_expanded = k.to(dtype=torch.complex128)[:, None, :]  # (N, 1, 3)
        AsT = self.As.T[None, :, :].to(dtype=torch.complex128)  # (1, T, 3)
    ...
    def integrands(self, k):
        ...
        As_T = self.As.permute(1, 0)[None, :, :].to(dtype=exp_int.dtype)  # (1, T, 3)
    ...
    def integral(self, k):
        ...
        # Multiply by dt to match the scale of the NumPy integration
        # integral_vec = integral_vec * dt
